...
/*   Updated: 2024/03/29 19:18:32 by lcadinot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	i_map(t_data *game, int fd)
{
	...
	if (check_path(game)) // check path pour les wall mais pas pour door
	...
	if (data_map_store(game, tmp_map) || dir_player(game, game->map.grid)
		...
		return (printf("dirplay-->%d\n", check_data_map(game->map.grid)),
			free_all(game), free_map_lst(ltmap),
